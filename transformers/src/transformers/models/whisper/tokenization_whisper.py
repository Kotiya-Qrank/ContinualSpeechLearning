...
TASK_IDS = ["translate", "transcribe",
            "RE", "EE", "KS", "SID", "ER", "IC", "SF", "IC_1", "IC_2", "IC_3",
            "NONE"]
...
class WhisperTokenizer(PreTrainedTokenizer):
    ...
    def prefix_tokens(self) -> List[int]:
        ...
        relation_extraction_token_id = self.convert_tokens_to_ids("<|RE|>")
        event_extraction_token_id = self.convert_tokens_to_ids("<|EE|>")
        keyword_spotting_token_id = self.convert_tokens_to_ids("<|KS|>")
        speaker_identification_token_id = self.convert_tokens_to_ids("<|SID|>")
        emotion_recognition_token_id = self.convert_tokens_to_ids("<|ER|>")
        intent_classification_token_id = self.convert_tokens_to_ids("<|IC|>")
        spoken_slot_filling_token_id = self.convert_tokens_to_ids("<|SF|>")
        intent_classification_token_id_1 = self.convert_tokens_to_ids("<|IC_1|>")
        intent_classification_token_id_2 = self.convert_tokens_to_ids("<|IC_2|>")
        intent_classification_token_id_3 = self.convert_tokens_to_ids("<|IC_3|>")
        ...
        if self.task is not None:
            if self.task == "translate":
                ...
                bos_sequence.append(relation_extraction_token_id)
            elif self.task == "EE":
                bos_sequence.append(event_extraction_token_id)
            elif self.task == "KS":
                bos_sequence.append(keyword_spotting_token_id)
            elif self.task == "SID":
                bos_sequence.append(speaker_identification_token_id)
            elif self.task == "ER":
                bos_sequence.append(emotion_recognition_token_id)
            elif self.task == "IC":
                bos_sequence.append(intent_classification_token_id)
            elif self.task == "SF":
                bos_sequence.append(spoken_slot_filling_token_id)
            elif self.task == "IC_1":
                bos_sequence.append(intent_classification_token_id_1)
            elif self.task == "IC_2":
                bos_sequence.append(intent_classification_token_id_2)
            elif self.task == "IC_3":
                bos_sequence.append(intent_classification_token_id_3)
            elif self.task == "NONE":
                bos_sequence = [bos_token_id]
            else:
                bos_sequence.append(self.convert_tokens_to_ids(self.task))
        if not self.predict_timestamps and not self.task == "NONE":
