...
class WhisperEncoder(WhisperPreTrainedModel):
    ...
    def __init__(self, config: WhisperConfig):
        ...
        self.weighted_sum_layer = config.weighted_sum_layer

        #############################################################################
        # self.weights = torch.nn.Parameter(torch.zeros([config.encoder_layers + 1]))  # 第1处改动
        config.num_tasks = 7
        self.multi_weights = torch.nn.Parameter(torch.zeros([config.num_tasks, config.encoder_layers + 1]))  # 为每个任务配置一个weighted sum layer
        self.temperature = 1
        #############################################################################
    ...
    def _freeze_parameters(self):
        ...
        # self.multi_weights.requires_grad = True
    ...
    def forward(
            self,
            input_features,
            input_tasks,    # 加入task信息，在batch内进行不同任务的选择
            attention_mask=None,
            head_mask=None,
        ...
        for idx, encoder_layer in enumerate(self.layers):
            norm_hidden_states.append(self.layer_norm(hidden_states))  # 第3处改动
        ...
        ###################################################################################################
        norm_hidden_states.append(hidden_states)
        stacked_feature = torch.stack(norm_hidden_states, dim=0)

        _, batch_size, *origin_shape = stacked_feature.shape
        stacked_feature = stacked_feature.view(len(self.layers) + 1, batch_size, -1)
        # norm_weights = torch.nn.functional.softmax(self.weights, dim=-1)
        norm_multi_weights = torch.nn.functional.softmax(self.multi_weights / self.temperature, dim=-1)
        norm_weights = (input_tasks @ norm_multi_weights).T

        weighted_feature = (norm_weights.unsqueeze(-1) * stacked_feature).sum(dim=0)
        weighted_feature = weighted_feature.view(batch_size, *origin_shape)
        if self.weighted_sum_layer:
            hidden_states = weighted_feature
        ###################################################################################################
        if output_hidden_states:
            encoder_states = encoder_states + (hidden_states,)
...
class WhisperModel(WhisperPreTrainedModel):
    ...
    def forward(
            ...
            input_tasks: Optional[torch.FloatTensor] = None,
            attention_mask: Optional[torch.LongTensor] = None,
...
class WhisperForConditionalGenerationWithWeightedSumLayer(WhisperPreTrainedModel):
    base_model_prefix = "model"
    ...
    def forward(
        ...
        if input_features is not None:
            input_tasks = torch.zeros([input_features.shape[0], 7]).cuda()
            for i, label in enumerate(labels):
                index = int(label[2])-51867
                if index == -1508: index = 5
                if index == -2: index = 6
                input_tasks[i, index] = 1
        ###################################################################
        ...
        if task is not None or language is not None or (forced_decoder_ids is None and prompt_ids is not None):
            ...
            if hasattr(generation_config, "task"):
                if generation_config.task in TASK_IDS:
                    if generation_config.task != "transcribe": generation_config.task = "<|" + generation_config.task + "|>"
